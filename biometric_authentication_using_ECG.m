...
plot(Orig_Sig)
[qrs_amp_raw,qrs_i_raw,delay]=pan_tompkin(Orig_Sig,200,1)
...
time=10;
f=fread(fid,2*360*time,'ubit12');
Orig_Sig1=f(1:2:length(f));
plot(Orig_Sig1)
[qrs_amp_raw1,qrs_i_raw1,delay1] = pan_tompkin(Orig_Sig1,200,1);

figure(5)
plot(qrs_amp_raw1,'linewidth',2,'Color','g')
figure(6)
plot(qrs_amp_raw,'linewidth',2,'color','r')

d=[];
d = xcorr(qrs_amp_raw,qrs_amp_raw1);
figure(7)
stem(d)
d
